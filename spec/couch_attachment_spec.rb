...
describe 'has_local_attached default_style' do
  before do
  ...
  it 'respects default_style in url' do
    obj = DefaultStyleTest.new
    obj.file_file_name = 'test.jpg'

    url = obj.file.url
    expect(url).to include('not_original.jpg')
    expect(url).not_to include('original')
  end

  it 'respects default_style in path' do
    obj = DefaultStyleTest.new
    obj.file_file_name = 'test.jpg'

    p = obj.file.path
    expect(p.to_s).to include('not_original.jpg')
    expect(p.to_s).not_to include('original')
  end

  it 'still allows explicit style override' do
    obj = DefaultStyleTest.new
    obj.file_file_name = 'test.jpg'

    expect(obj.file.url(:thumb)).to include('thumb.jpg')
    expect(obj.file.path(:thumb).to_s).to include('thumb.jpg')
  end
end
